...
Сейчас мы хотим добавить информацию о каждом запросе к серверу и авторизации в логи. Не рекомендуется включать функцию **log** во все возможные месте. У зон есть функция *print* которая выводит сообщение как строку. Сообщение функции *print* всплывает вверх по иерархии вложенных зон пока одна из родительских зон не перехватит его или не попадёт на самый вверх в зону *root*. Таким образом, нам нужно только переписать функцию *print* в классе **ZoneSpecification** и передавать сообщения для логирования. Мы создадим новую *zoneSpecification* c функцией для перехвата и вызова функции *log*.
